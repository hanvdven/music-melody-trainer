...
- WHOLE notes (duration 48) instead of quarters; SUS removed as a complexity option (`CHORD_COMPLEXITY`
  + `COMPLEXITY_NOTES`); the exotic label spelled out `'Altered / Extended'` (renders ALL-CAPS); the
  low-C ledger suppressed via a new `suppressLedgers` flag threaded `MiniMelody → MelodyNotesLayer →
...
Both carousels' options carry an ALL-CAPS label (Han 2026-07-26 UAT): chord notation =
...
**Final layout (Han 2026-07-30):** every control is the SAME shared `CarouselField` (Han rejected an
earlier attempt that turned the three #502 controls into bespoke compact `LeftFanCarousel` fans crammed
on the top staff — "waarom hergebruik je niet wat reeds bestaat?"). The three adjustment controls are
"boven elkaar" = ONE PER STAFF, stacked vertically on the RIGHT of the scheme carousel, each with a
STAFF-HEIGHT icon like the other setters: HIGHLIGHTS on the treble staff, ANIMATION on the bass staff,
LYRICS on the percussion staff (`controlCx = startX + 0.84·(endX−startX)`, icon 30). The THEME carousel
sits centred on the BASS staff (under the scheme). The FONT carousel (#532) will be centred on the
percussion staff (needs the app-wide `--app-text-font` mechanism first).
...
- **Disco:** a `<canvas>` mirror-ball (`components/layout/DiscoBackground.jsx`), NOT CSS — CSS cannot give
  a particle a position-dependent speed. Specks are a beam-on-a-flat-wall projection `x = cx + K·tan(θ)`,
  `θ = phase + ω·t` wrapped to (−π/2, π/2): SLOW at the centre, FAST toward the edges, sweeping one
  direction (exit right, re-enter left — no bounce). 5 rows at 15/35/50/65/85 % with 4/5/6/5/4 dots; small
  blurred rectangles, faded near the edge. Coloured R/B/G/Y lamps of varied size drift additively
  (`globalCompositeOperation:'lighter'`) and leave frame. Mounted only for the disco theme as the first
  child of `.app-top-wrapper` (made `isolate` so the `z-index:-1` `.disco-canvas` sits behind the
  transparent header + sheet). rAF-driven, no per-frame React state (§6).
