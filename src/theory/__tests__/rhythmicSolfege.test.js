...
describe('getTupletSyllable — irregular divisions', () => {
    ...
    it('sextuplet (n=6): ta va ki di da ma — compound subdivision, generated by 6:4', () => {
        const s = ['ta', 'va', 'ki', 'di', 'da', 'ma'];
        s.forEach((syl, i) => expect(getTupletSyllable(i, 6)).toBe(syl));
    });

    it('octuplet (n=8): split 4+4 → ta ka di mi | ta ka di mi', () => {
        const s = ['ta', 'ka', 'di', 'mi', 'ta', 'ka', 'di', 'mi'];
        s.forEach((syl, i) => expect(getTupletSyllable(i, 8)).toBe(syl));
    });

    it('nonuplet (n=9): split 3+3+3 → ta ki da | ta ki da | ta ki da (generated by 9:8)', () => {
        const s = ['ta', 'ki', 'da', 'ta', 'ki', 'da', 'ta', 'ki', 'da'];
        s.forEach((syl, i) => expect(getTupletSyllable(i, 9)).toBe(syl));
    });

    it('returns null for out-of-range noteCount (e.g. 10)', () => {
        expect(getTupletSyllable(0, 10)).toBeNull();
    });
});
