 ...
 * Indexed by noteCount (2–9); position 0 is always 'ta' (beat onset).
 *
 * ≤7: directly from the article's ta-ka-di-mi / ta-va-ki-di-da-ma families.
 * 8:  split 4+4  → ta ka di mi | ta ka di mi  (simple ×2)
 * 9:  split 3+3+3 → ta ki da | ta ki da | ta ki da  (compound ×3)
 *   — "For noteCount > 7, split near halfway and recurse" (Hoffman et al. 1996)
 *   Note: n=8 is not generated by the current engine (not in TUPLET_DEFS) but is
 *   included for completeness. n=9 (nonuplet, 9:8) is in TUPLET_DEFS and is generated.
 */
const TUPLET_SYLLABLES = {
    ...
    9: ['ta', 'ki', 'da', 'ta', 'ki', 'da', 'ta', 'ki', 'da'], // 3+3+3
};
