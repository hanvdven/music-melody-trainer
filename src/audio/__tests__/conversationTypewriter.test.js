...
import { buildTypewriterSchedule, pickLowercaseTone } from '../conversationTypewriter';
...
describe('conversationTypewriter (#922)', () => {
    ...
    it('uppercase letters and punctuation always resolve to C3', () => {
        const schedule = buildTypewriterSchedule('A!', 4);
        expect(schedule[0].tone).toBe('C3');
        expect(schedule[1].tone).toBe('C3');
    });

    it('lowercase letters use the weighted C3/D3/E3 pool, never anything else', () => {
    ...
    it('pickLowercaseTone respects the 70/25/5 weighting at its boundaries', () => {
        expect(pickLowercaseTone(() => 0)).toBe('C3');
        expect(pickLowercaseTone(() => 0.69)).toBe('C3');
        expect(pickLowercaseTone(() => 0.71)).toBe('D3');
        expect(pickLowercaseTone(() => 0.94)).toBe('D3');
