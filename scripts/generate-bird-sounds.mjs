...
// as plain { notes, durations, offsets } triplets in this app's own tick units (TICKS_PER_WHOLE=48) — a
// \`notes[i]\` entry is a chord (array of note names) when the source has simultaneous notes there, a plain
// string otherwise (all lines kept, not reduced to unison). Tempo-agnostic (rescales automatically to
// whatever bpm useWorldAmbientMusic.js plays them at). useWorldAmbientMusic.js picks a random layer per
// trigger, at most 3 concurrent (Han: "speel altijd maximaal 3").
