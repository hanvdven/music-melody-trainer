...
// nodig, zodat je snel kan laden/vinden in de files"): this file crossed ~1900 lines as the asset library
// grew (500+ files now). A full multi-file split was considered but rejected FOR NOW — every override table
// below reads/writes the SAME shared helpers (`scanRows`, `rowCells`, `analyze`, …) and the main scan loop
// depends on their declaration order; splitting it apart under time pressure risks a subtle breakage that's
// hard to spot (this session already found 2 such ordering bugs — see §690/§691's "ran before stripBrand"
// and "match-all vs match-one" fixes). Lower-risk, immediate fix instead: the SECTION banners below (search
// for "═══ SECTION" to jump between them) plus this table of contents, so any concern below has a fast,
// unambiguous place to land without reading the whole file top to bottom. If you (Han) want the heavier
// multi-file split anyway, say so explicitly and it'll get its own careful, tested pass.
//
...
//   SECTION: PER-CREATURE ANIMATION DEFS      — bespoke animation-cell layouts for one specific creature
//                                                (horse, goblin, zombie, maid, skeleton, knighty, …)
...
//   SECTION: SPECIAL ONE-OFF EXPANSIONS       — boss_spider/large skull/devil/damned tree/critters/evil
//                                                wizard: each its own "one file → N manifest entries" shape
...
// ═══ SECTION: PER-CREATURE ANIMATION DEFS ═══
function horseAnimations() {
...
// `seq()` pattern used elsewhere to plain start/end frame numbers, avoiding manual row/col transcription.
const frameRange = (startN, endN, cols) => {
    const cells = [];
...
function maidSwordDownAnimations() {
...
function poopThrowerAnimations() {
...
function artLadyAnimations(contentRows) {
...
function santaVampireAnimations() {
    return [
...
function knightyAnimations(totalRows = 14) {
    const seq = (...pairs) => pairs.flatMap(([row1, cols1]) => cols1.map((c) => ({ row: row1 - 1, col: c - 1 })));
...
function knightyRunFastAnimations() {
...
function knightHeavySheet2Animations() {
...
function boss_spiderAnimations(contentRows) {
...
function largeSkullAnimations(contentRows) {
...
// the last.
function devilAnimations(contentRows) {
