...
async function loadProjectInfo() {
  try {
    ...
    if (info.projectName) {
      ...
      // Auto-select the launching project on first visit so "Add task" doesn't
      // fail with "Select a project first". Only fills in when nothing is
      // already remembered \u2014 user choice via the dropdown still wins.
      if (!currentProject) {
